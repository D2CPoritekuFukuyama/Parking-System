...
#include "CateNumMatching.hpp"
#define NUMBER 10
using namespace std;

CateNumMatching::CateNumMatching(IplImage *target_img) : Matching(target_img){
    
}

string CateNumMatching::CategoryNum_Matching(){
    ...
    for(int i = 0; i < NUMBER; i ++){
        ...
        if(max_val >= 0.7)
        {
            ...
            num_map[max_loc.x] = i;
            cvSaveImage("image/test2.jpg", target_img);
            //同じ数字をもう一度検索
            --i;
            ++ count;
        }
